...
\shorttitle{Claim-gated service-menu optimization}
\shortauthors{J. Ana}

\title[mode=title]{Claim-Gated Dynamic Service-Menu Optimization for Many-to-One Demand-Responsive Transit}
...
and the package-level claim gates that govern manuscript interpretation. In
the current generated package, the claim-ready flag, strict claim-guard flag,
and positive-claims permission flag are all false. Positive empirical claims
about adaptive-menu effects, product ablations, adaptive-window increments,
menu-construction value, exact/greedy computational credibility, and semi-real
case evidence remain blocked by readiness and strict claim-guard status. The
contribution is therefore a claim-gated service-menu optimization framework
and a transparent diagnostic evidence architecture for future claim-ready
evaluation.
\end{abstract}

\begin{keywords}
demand-responsive transit \sep service-menu optimization \sep meeting points \sep pickup time windows \sep multinomial logit \sep paired replay \sep artifact gates \sep transportation operations
\end{keywords}
...
\citep{cordeau2007dial,ho2018survey}. In a last-mile or feeder setting, a
platform can sometimes serve a passenger at home, sometimes ask the passenger
to walk to a meeting point, and sometimes decline to serve the request if
service is unattractive or infeasible. Meeting points can reduce routing burden
and improve consolidation, but they also impose walking and timing burdens on
passengers \citep{stiglic2015benefits,zheng2019meeting,fielbaum2021ondemand,cortenbach2024}.
These outcomes are shaped not only by routing cost, but also by what the
passenger is shown. A displayed alternative is a service product: it tells the
passenger where to be picked up, when pickup is expected, and what price
applies.
...
The paper is written for \emph{Transportation Research Part E} as a service
...
\citep{cordeau2007dial,parragh2008survey,ho2018survey}. Meeting-point DRT
extends this setting by allowing passengers to walk to shared pickup points,
which can reduce route burden and improve consolidation
\citep{stiglic2015benefits,zheng2019meeting,fielbaum2021ondemand,cortenbach2024}.
In the present paper, meeting points are not treated as route nodes alone.
They are components of displayed service bundles, and their operational value
depends on time-window treatment, price, and passenger response.
...
substitute or choose an outside option \citep{talluri2004revenue,rusmevichientong2010dynamic,desir2022capacitated}.
A DRT service menu has a related choice structure, but each product is
state-dependent and operationally endogenous. Passenger response is represented
with a multinomial-logit structure grounded in the discrete-choice literature
\citep{mcfadden1974conditional,ben1985discrete}. The cost of a bundle depends
on the current route, capacity, pickup location, time window, and feasibility
checks. The menu decision is therefore both a display decision and a
transportation operations decision.
...
provenance and reproducibility controls. Decision-focused and
predict-then-optimize pipelines underline why predictive and optimization
components should be evaluated through the downstream decision they support
\citep{elmachtoub2022smart}. The current manuscript treats paired replay,
checkpoint load status, normalized rows, artifact status, package status, and
strict claim guards as first-class evidence controls. This framing does not
replace empirical evidence. It prevents a manuscript from overstating what the
generated evidence can support and clarifies what future work must regenerate
before stronger claims are available.
...
Let $\mathcal{I}$ be the sequence of requests. For request $i\in\mathcal{I}$,
let $S_i$ denote the system state before the menu is displayed. Let $L_i$ be
the feasible pickup-location set, including home service and any feasible
meeting-point alternatives. For each $m\in L_i$, let $W_i(m)$ be the feasible
set of pickup-window treatments, and let $P_i(m,w)$ be the feasible set of
prices for location-window pair $(m,w)$.

The candidate bundle set is
\begin{align}
B_i=\{b=(m,w,p):\;&m\in L_i,\; w\in W_i(m),\; p\in P_i(m,w), \nonumber\\
& b\in \mathcal{F}_i(S_i)\},
\end{align}
where $\mathcal{F}_i(S_i)$ is the set of route-, capacity-, ETA/window-, and
guardrail-feasible bundles under state $S_i$.
The platform chooses binary display variables $x_{ib}$, where $x_{ib}=1$ if
bundle $b$ is displayed. The menu is
\begin{equation}
M_i = \{b\in B_i:x_{ib}=1\},
\end{equation}
with menu-size constraint
\begin{equation}
\sum_{b\in B_i}x_{ib}\le K.
\end{equation}
...
the outside option. Let $U_{ib}$ be the systematic utility of displayed bundle
$b$:
\begin{equation}
U_{ib}=\alpha_0+\alpha_p p_{ib}+\alpha_d d_{ib}+\alpha_t t_{ib}
       +\alpha_w q_{ib}+\alpha_h h_{ib},
\end{equation}
where $d_{ib}$ is walking distance, $t_{ib}$ is pickup-time or in-vehicle-time
burden, $q_{ib}$ summarizes time-window compatibility, and $h_{ib}$ indicates
home service. Let $U_{i0}$ denote outside-option utility. The probability of
choosing displayed bundle $b$ is
\begin{equation}
\Pr_i(b\mid M_i)=
\frac{\exp(U_{ib})}{\exp(U_{i0})+\sum_{j\in M_i}\exp(U_{ij})}.
\end{equation}
The outside-option probability is
\begin{equation}
\Pr_i(0\mid M_i)=
\frac{\exp(U_{i0})}{\exp(U_{i0})+\sum_{j\in M_i}\exp(U_{ij})}.
\end{equation}

The expected one-request menu objective can be written as
\begin{align}
\max_x\quad
&\sum_{b\in B_i}x_{ib}\Pr_i(b\mid M_i)\left[p_{ib}-c_{ib}(S_i)\right] \nonumber\\
&-\lambda_{\mathrm{out}}\Pr_i(0\mid M_i)-\Phi(M_i,S_i),
\end{align}
where $c_{ib}(S_i)$ is predicted operational cost, $\lambda_{\mathrm{out}}$
is an opt-out/lost-demand penalty, and $\Phi(M_i,S_i)$ collects
service-quality, ETA/window, and guardrail penalties. This representation is
intentionally general: concrete runtime policies can use different
menu-objective modes, pricing modes, and solver contracts while preserving the
same evidence accounting.
...
\begin{table*}[t]
\centering
\caption{Diagnostic service-menu evaluation pipeline.}
...
7 & Select a displayed menu subject to menu-size and policy contracts. Use exact enumeration when candidate count is inside the exact limit; use greedy fallback when candidate count exceeds that limit; record solver diagnostics, candidate counts, fallback reason, build time, and selected menu metadata. \\
8 & Evaluate the MNL response model over displayed bundles plus outside option. \\
9 & Apply the selected outcome: accepted home pickup mutates the route through home service; accepted meeting-point pickup mutates the route through the chosen meeting point; opt-out records lost demand and does not mutate the route. \\
10 & Emit normalized row fields, checkpoint load status, policy tag, method metadata, manifest/settings hashes, and outcome counts. \\
11 & Classify generated artifacts through readiness, row, artifact, package, and strict claim-guard gates. \\
\bottomrule
\end{tabular}
\end{table*}
...
effects with changes in scenario, checkpoint provenance, or replay conditions.

\begin{table*}[t]
\centering
\caption{Primary policy family and paired-comparison roles.}
\begin{tabular}{@{}p{0.30\textwidth}p{0.62\textwidth}@{}}
\toprule
Policy tag & Role in paired comparison \\
\midrule
\code{mainline_no_menu} & Baseline slot without displayed service-menu construction \\
\code{mainline_fixed_menu} & Fixed service-menu slot \\
\code{mainline_random_menu} & Random displayed-menu diagnostic slot \\
\code{mainline_optimized_m} & Optimized meeting-point-only product slot \\
\code{mainline_optimized_mw} & Optimized meeting-point and window product slot \\
\code{mainline_optimized_fixed_window} & Optimized menu with fixed-window handling \\
\code{mainline_optimized_adaptive} & Optimized menu with adaptive-window handling \\
\bottomrule
\end{tabular}
\end{table*}
...
The normalized rows record profit and service metrics, including
\code{net_profit}, \code{acceptance_rate}, \code{optout_rate}, accepted home
count, accepted meeting-point count, home share, meeting-point uptake, served
rate, and status fields. Provenance fields include policy tag, manifest hash,
settings hash, checkpoint path, checkpoint hash, checkpoint requirement, and
\code{checkpoint_load_status}. The checkpoint fields are not administrative
...
The Results section begins with the strict claim gate. In the current package,
the claim-ready flag, strict claim-guard flag, and positive-claims permission
flag are all false. It contains 74 package artifacts, 70 existing artifacts, 4
missing artifacts, and 108 blockers. The package is usable for diagnostic
interpretation and provenance transparency, but it is not claim-ready
empirical evidence.

\begin{table*}[t]
\centering
\caption{Claim status and allowed manuscript use under the current strict guard.}
\small
\begin{tabular}{@{}p{0.22\textwidth}p{0.20\textwidth}p{0.50\textwidth}@{}}
\toprule
Claim & Current status & Allowed manuscript use \\
\midrule
C1 adaptive menu & unsupported blocked & Not allowed as a positive claim; status and blockers only \\
C2 product ablation & diagnostic blocked & Diagnostic structure and future upgrade conditions only \\
C3 adaptive window & unsupported & Comparison slot only; no directional effect language \\
C4 menu construction & diagnostic blocked & Auditable mechanism discussion only \\
C5 ETA boundary & diagnostic only & ETA/no-filter boundary discussion only \\
C6 exact/greedy & blocked diagnostic & Computational diagnostic appendix only \\
C7 provenance & status supported & Provenance/status transparency only \\
C8 semi-real case & scaffold blocked & Future-study scaffold only \\
\bottomrule
\end{tabular}
\end{table*}
...
has \code{claim_ready=false}, and the guard blocks positive claims for C1,
C2, C3, C4, C6, and C8. The manuscript therefore cannot conclude that one menu
family has claim-ready performance effects over another. It can only state the
...
does not claim such behavior because C8 remains scaffold-only. If a reviewer
asks whether exact/greedy material establishes solver quality, the answer is
that C6 is blocked diagnostic and future evidence is required.
...
The current manuscript remains conditional diagnostic with
\code{claim_ready=false}. It provides a service-menu formulation, paired-replay
evaluation architecture, and transparent claim-boundary audit. Positive
empirical claims remain blocked by readiness and strict claim-guard status,
and final replay was not authorized in the current milestone. The paper is
therefore a claim-gated diagnostic manuscript and a foundation for future
clean evidence regeneration.

\appendix

\section{Source Map And Claim Audit}

The manuscript uses
\path{manuscript/TR_E_WORK2_TABLE_FIGURE_SOURCE_MAP.md} as the source map for
planned manuscript objects. Each object is required to identify source
artifact path, claim ID, claim status, allowed manuscript use, and evidence
class. The strict claim boundary is documented in
\path{manuscript/TR_E_WORK2_CLAIM_AUDIT.md} and the generated
\path{work2_coding/artifacts/work2_robust_menu/phase10_paper_artifacts/CLAIM_GUARD.json}.

\section{ETA And No-Filter Diagnostic Boundary}

ETA/no-filter material belongs to C5\_eta\_robustness\_boundary and is
...
\section{Exact/Greedy Computational Diagnostics}
...
provisional, and C6\_exact\_greedy\_computational\_credibility remains
blocked. The appendix may report computational status only within that
boundary.

\section{Case-Scaffold Boundary}

Case-study material under \code{.planning/data/case_studies/} is
scaffold-only. It may define future source contracts, route-selection ideas,
and demand protocols. It cannot be presented as executed case evidence, real
passenger evidence, or claim-ready validation in the current manuscript.

\section{Prohibited-Language Check}

The revised prohibited-language check is maintained in
\path{manuscript/TR_E_WORK2_REVISED_PROHIBITED_LANGUAGE_CHECK.md}. Hits from
the scan must be classified as safe blocked-claim/status discussion, safe
explicit denial, or unsafe language requiring manuscript revision. The final
status is pass only when no unqualified positive claim remains.
