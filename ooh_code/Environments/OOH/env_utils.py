...
class utils_env(object):
    ...
    def fleet_travel_cost(self, fleet, depot):
        total_cost = 0.0
        for vehicle in fleet["fleet"]:
            ...
            has_depot_endpoints = (
                len(route) >= 2
                and route[0].id_num == depot.id_num
                and route[-1].id_num == depot.id_num
            )
            if has_depot_endpoints:
                for idx in range(len(route) - 1):
                    total_cost += self._edge_cost(route[idx], route[idx + 1])
            else:
                total_cost += self._edge_cost(depot, route[0])
                for idx in range(len(route) - 1):
                    total_cost += self._edge_cost(route[idx], route[idx + 1])
                total_cost += self._edge_cost(route[-1], depot)
    ...
    def reopt_HGS(self, data, fallback_fleet=None, depot=None):
        customer_count = max(len(data['x_coordinates']) - 1, 0)
        if customer_count <= 1 and fallback_fleet is not None and depot is not None:
            return fallback_fleet, self.fleet_travel_cost(fallback_fleet, depot)
